...
from .models import Blog, Content, Category, Tag, Comment
...
class ContentInline(SortableInlineAdminMixin, admin.StackedInline):
    ...
    fields = ('collapse_button', 'order', 'id', 'content_type', 'text_content', 'image_url', 'link_url')
    readonly_fields = ('drag_handle', 'collapse_button')  # Make drag handle and collapse button read-only
    ...
    def collapse_button(self, obj=None):
        ...
        if obj:
            ...
            if obj.content_type == "paragraph":
                content_preview = obj.text_content[:50] + "..." if obj.text_content else "No content"
            elif obj.content_type == "quote":
                content_preview = f'"{obj.text_content[:50]}"' if obj.text_content else "No quote"
                ...
                content_preview = obj.text_content[:50] if obj.text_content else "No heading"
